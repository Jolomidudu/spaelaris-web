...
const [activeCategory, setActiveCategory] = useState("All");
  ...
  useEffect(() => {
    ...
    if (savedLocation) {
      const updateLocation = window.setTimeout(() => {
        setLocation(savedLocation);
      }, 0);

      return () => window.clearTimeout(updateLocation);
    }
  }, []);
...
>
          <div
            className={`flex h-[92px] w-full max-w-[124px] items-center justify-center rounded-[28px] border transition lg:max-w-none ${
              active
...
/>
          ...
          <span
            className={`mt-3 max-w-[120px] text-[14px] leading-5 lg:max-w-none ${
      ...
      <section className="mt-12">
        ...
        <div className="mt-5 flex gap-5 overflow-x-auto px-5 pb-5 [scrollbar-width:none] sm:px-8 lg:mx-auto lg:grid lg:max-w-7xl lg:grid-cols-3 lg:overflow-visible">
          {recommendations.map((item) => (
            <Link
              href="/services"
              key={item.title}
              className="group min-w-[320px] max-w-[320px] flex-none sm:min-w-[370px] sm:max-w-[370px] lg:min-w-0 lg:max-w-none"
            >
